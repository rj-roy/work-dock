...
export default function AsideNav({ activeCategory }: MenuNavProps) {
    const [isOpen, setIsOpen] = useState(false);
    const active = activeCategory ?? 'all';

    const menuCategories = [
        { name: 'All', href: '/find-space', icon: HandPlatter },
        { name: 'Private Office', href: '/find-space?category=private-office', icon: UtensilsCrossed },
        { name: 'Hot Desk', href: '/find-space?category=hot-desk', icon: ChefHat },
        { name: 'Meeting Room', href: '/find-space?category=meeting-room', icon: Wine },
        { name: 'Studio', href: '/find-space?category=studio', icon: IceCream },
    ];
    ...
    return (
        <aside className='fixed z-99'>
            <div className='relative w-full md:min-h-screen md:shadow-sm'>
                {
                    isOpen && (
                        <button
                            type="button"
                ...
                {
                    isOpen &&
                    <nav className={`fixed left-0 top-0 z-40 border-r border-r-primary/20 flex h-screen min-w-60 flex-col overflow-y-auto shadow-xl transition-transform duration-200 md:static md:h-screen md:w-full md:translate-x-0 md:flex md:shadow-none ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                        {/* Header */}
                        ...
                        <div className="flex-1 px-4 space-y-1">
                            {menuCategories.map((category) => {
                                ...
                                const isActive = active === category.name.toLowerCase().replace(/\s+/g, '-');
                                ...
                                return (
                                    <Link
                                        ...
                                        className={`flex items-center gap-3 px-4 py-3.5 rounded-lg text-sm font-medium transition-all duration-200 ${isActive
                                            ...
                                            : 'hover:bg-neutral/10 dark:hover:bg-neutral/20'
                                            }`}
                                    >
