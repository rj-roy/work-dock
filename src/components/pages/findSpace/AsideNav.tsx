'use client'
import Link from 'next/link';
import {
    UtensilsCrossed,
    ChefHat,
    Wine,
    IceCream,
    ShoppingCart,
    HandPlatter,
    ChevronLeft,
    ChevronRight,
} from 'lucide-react';
import { useState } from 'react';

type MenuNavProps = {
    activeCategory: string;
};

export default function AsideNav({ activeCategory }: MenuNavProps) {
    const [isOpen, setIsOpen] = useState(false);
    const active = activeCategory ?? 'all';

    const menuCategories = [
        { name: 'All', href: '/find-space', icon: HandPlatter },
        { name: 'Private Office', href: '/find-space?category=private-office', icon: UtensilsCrossed },
        { name: 'Hot Desk', href: '/find-space?category=hot-desk', icon: ChefHat },
        { name: 'Meeting Room', href: '/find-space?category=meeting-room', icon: Wine },
        { name: 'Studio', href: '/find-space?category=studio', icon: IceCream },
    ];

    return (
        <aside className="fixed z-60">
            <div className="relative w-full md:shadow-sm">

                {isOpen && (
                    <button
                        type="button"
                        onClick={() => setIsOpen((prev) => !prev)}
                        className="fixed left-60 top-100 bottom-100 z-900 flex h-10 w-10 items-center justify-center rounded-full border border-neutral/20 bg-primary text-white shadow-sm backdrop-blur"
                    >
                        <ChevronLeft className="h-5 w-5" />
                    </button>
                )}

                {!isOpen && (
                    <button
                        type="button"
                        onClick={() => setIsOpen((prev) => !prev)}
                        className="fixed left-3 top-100 bottom-100 z-70 flex h-10 w-10 items-center justify-center rounded-full border border-neutral/20 bg-primary text-white shadow-sm backdrop-blur"
                    >
                        <ChevronRight className="h-5 w-5" />
                    </button>
                )}

                <nav
                    className={`fixed left-0 top-0 z-95 flex h-dvh w-64 flex-col overflow-y-auto border-r border-r-primary/20 shadow-xl transition-transform duration-200 bg-white dark:bg-black ${isOpen ? 'translate-x-0' : '-translate-x-full'
                        }`}
                >
                    <div className="px-2 pt-8 pb-6">
                        <h1 className="text-xl font-serif font-bold text-primary">
                            Work Dock
                        </h1>
                        <p className="text-sm text-neutral dark:text-neutral/80 mt-1">
                            Editorial Menu
                        </p>
                    </div>

                    <div className="flex-1 px-4 space-y-1">
                        {menuCategories.map((category) => {
                            const Icon = category.icon;
                            const isActive = active === category.name.toLowerCase().replace(/\s+/g, '-');

                            return (
                                <Link
                                    key={category.name}
                                    href={category.href}
                                    onClick={() => setIsOpen(false)}
                                    className={`flex items-center gap-3 px-4 py-3.5 rounded-lg text-sm font-medium transition-all duration-200 ${isActive
                                        ? 'bg-primary font-bold shadow-md text-white'
                                        : 'hover:bg-neutral/10 dark:hover:bg-neutral/20'
                                        }`}
                                >
                                    <Icon className="w-5 h-5" />
                                    <span>{category.name}</span>
                                </Link>
                            );
                        })}
                    </div>

                    <div className="px-4 pb-8 space-y-4">
                        <div className="border-t border-neutral/30 dark:border-neutral/50" />

                        <Link
                            href="/cart"
                            onClick={() => setIsOpen(false)}
                            className="flex items-center justify-center gap-2 w-full bg-primary hover:bg-primary/90 text-tertiary dark:text-secondary px-4 py-3.5 rounded-lg font-semibold transition-all duration-200 shadow-md hover:shadow-lg"
                        >
                            <ShoppingCart className="w-5 h-5" />
                            <span>View Cart</span>
                        </Link>
                    </div>
                </nav>

                {isOpen && (
                    <div
                        onClick={() => setIsOpen(false)}
                        className="fixed inset-0 z-64 bg-black/40"
                    />
                )}
            </div>
        </aside>
    );
}